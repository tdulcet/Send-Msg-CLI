...
def examples():
    print("Sendpy Examples:\n"+
        "    Send e-mail\n"+
        f'    $ {sendpy} -s \"Example\" -t \"Example <example@example.com>\"\n\n'+

        "    Send e-mail with message\n"+
        f'    $ {sendpy} -s \"Example\" -m \"This is an example!\" -t \"Example <example@example.com>\"\n\n'+

        "    Send e-mail with message and single attachment\n"+
        f'    $ {sendpy} -s \"Example\" -m \"This is an example!" -a "example.txt" -t \"Example <example@example.com>\"\n\n'+

        "    Send e-mail with message and multiple attachments\n"+
        f'    $ {sendpy} -s \"Example\" -m \"This is an example!\" -a "example1.txt" -a example2.txt -t \"Example <example@example.com>\"\n\n'+

        "    Send e-mail to a CC address\n"+
        f'    $ {sendpy} -s \"Example\" -t \"Example 1 <example1@example.com>\" -c \"Example 2 <example2@example.com>\"\n\n'+

        "    Send e-mail with a From address\n"+
        f'    $ {sendpy} -s \"Example\" -f \"Example <example@example.com>\" -t \"Example <example@example.com>\"\n\n'+

        "    Send high priority e-mail\n"+
        f'    $ {sendpy} -s \"Example\" -f \"Example <example@example.com>\" -S \"mail.example.com\" -u \"example\" -p \"password\" -P \"1 (Highest)\" -t \"Example <example@example.com>\"\n\n'+

        "    Send e-mail digitally signed with an S/MIME Certificate\n"+
        f'    $ {sendpy} -s \"Example\" -f \"Example <example@example.com>\" -S \"mail.example.com\" -u \"example\" -p \"password\" -C \"cert.p12\" -t \"Example <example@example.com>\"\n\n'+

        "    Send e-mail digitally signed with PGP/MIME\n"+
        f'    $ {sendpy} -s \"Example\" -f \"Example <example@example.com>\" -S \"mail.example.com\" -u \"example\" -p \"password\" -k \"passphrase\" -t \"Example <example@example.com>\"'"\n")
