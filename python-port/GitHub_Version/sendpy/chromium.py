...
def printColor(value, index, color):
    if value[index] == "expectValid":
        print(value[0], green + value[index] + reset)
    else:
        print(value[0], red + value[index] + reset)
        print(value[0], red + value[2] + reset)

def printValues(chromium_values):
    for i in chromium_values:
        length = len(i)
        if length == 3:
